...
@Database(entities = [TransactionEntity::class, CreditCardEntity::class, IncomeEntity::class, RecurringTransactionEntity::class], version = 7, exportSchema = true)
    ...
    companion object {
        val MIGRATION_6_7 = object : Migration(6, 7) {
            override fun migrate(db: SupportSQLiteDatabase) {
                // 1. Migrate transactions table (amount: TEXT -> INTEGER)
                db.execSQL("CREATE TABLE `transactions_new` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `description` TEXT NOT NULL, `amount` INTEGER NOT NULL, `date` INTEGER NOT NULL, `category` TEXT NOT NULL, `isCreditCard` INTEGER NOT NULL, `accountName` TEXT NOT NULL, `targetCreditCardId` INTEGER)")
                // Conversion from TEXT BigDecimal to Long cents: CAST(amount AS REAL) * 100
                // Using round to ensure precision during cast if necessary, though TEXT is deterministic.
                db.execSQL("INSERT INTO `transactions_new` (`id`, `description`, `amount`, `date`, `category`, `isCreditCard`, `accountName`, `targetCreditCardId`) SELECT `id`, `description`, CAST(ROUND(CAST(`amount` AS REAL) * 100) AS INTEGER), `date`, `category`, `isCreditCard`, `accountName`, `targetCreditCardId` FROM `transactions`")
                db.execSQL("DROP TABLE `transactions`")
                db.execSQL("ALTER TABLE `transactions_new` RENAME TO `transactions`")

                // 2. Migrate credit_cards table (creditLimit: TEXT -> INTEGER, balance: TEXT -> INTEGER)
                db.execSQL("CREATE TABLE `credit_cards_new` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `name` TEXT NOT NULL, `billingDay` INTEGER NOT NULL, `daysAfterBillingDate` INTEGER NOT NULL, `creditLimit` INTEGER NOT NULL, `balance` INTEGER NOT NULL)")
                db.execSQL("INSERT INTO `credit_cards_new` (`id`, `name`, `billingDay`, `daysAfterBillingDate`, `creditLimit`, `balance`) SELECT `id`, `name`, `billingDay`, `daysAfterBillingDate`, CAST(ROUND(CAST(`creditLimit` AS REAL) * 100) AS INTEGER), CAST(ROUND(CAST(`balance` AS REAL) * 100) AS INTEGER) FROM `credit_cards`")
                db.execSQL("DROP TABLE `credit_cards`")
                db.execSQL("ALTER TABLE `credit_cards_new` RENAME TO `credit_cards`")

                // 3. Migrate income table (amount: TEXT -> INTEGER)
                db.execSQL("CREATE TABLE `income_new` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `description` TEXT NOT NULL, `amount` INTEGER NOT NULL, `date` INTEGER NOT NULL)")
                db.execSQL("INSERT INTO `income_new` (`id`, `description`, `amount`, `date`) SELECT `id`, `description`, CAST(ROUND(CAST(`amount` AS REAL) * 100) AS INTEGER), `date` FROM `income`")
                db.execSQL("DROP TABLE `income`")
                db.execSQL("ALTER TABLE `income_new` RENAME TO `income`")

                // 4. Migrate recurring_transactions table (amount: TEXT -> INTEGER)
                db.execSQL("CREATE TABLE `recurring_transactions_new` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `description` TEXT NOT NULL, `amount` INTEGER NOT NULL, `category` TEXT NOT NULL, `isCreditCard` INTEGER NOT NULL, `accountName` TEXT NOT NULL, `frequency` TEXT NOT NULL, `startDate` INTEGER NOT NULL, `nextOccurrence` INTEGER NOT NULL)")
                db.execSQL("INSERT INTO `recurring_transactions_new` (`id`, `description`, `amount`, `category`, `isCreditCard`, `accountName`, `frequency`, `startDate`, `nextOccurrence`) SELECT `id`, `description`, CAST(ROUND(CAST(`amount` AS REAL) * 100) AS INTEGER), `category`, `isCreditCard`, `accountName`, `frequency`, `startDate`, `nextOccurrence` FROM `recurring_transactions`")
